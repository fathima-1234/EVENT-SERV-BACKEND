...
class ServicerRegister(APIView):
    def post(self, request):
        ...
        servicer = Servicer(
            full_name=full_name, email=email, password=password, mobile_no=mobile_no
        )
        servicer.save()

        servicer_id = servicer.id
        return Response(
            {"message": "Servicer registered successfully", "servicer_id": servicer_id},
        ...
        if servicer:
            if servicer.is_active:
                ...
                return Response(
                    {
                        "bool": False,
                        "message": "Your account is not active. Please wait for approval.",
                    }
                )
        else:
...
class UnblockServicerView(APIView):
    def put(self, request, Servicer_id):
        try:
            ...
            return Response(
                {"message": "Servicer not found"}, status=status.HTTP_404_NOT_FOUND
            )
